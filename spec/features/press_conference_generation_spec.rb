...
RSpec.describe 'Press Conference Generation', type: :feature do
  ...
  describe 'Basic press conference creation' do
    context 'as league owner' do
      ...
      it 'triggers ChatGPT response generation in background' do
        # Mock the job to avoid actual execution
        allow(ChatgptResponseGenerationJob).to receive(:perform_later)
        ...
        press_conference = PressConference.last
        expect(ChatgptResponseGenerationJob).to have_received(:perform_later).with(press_conference.id)
      end
